...
import gov.ifms.dmo.service.impl.DMOGuarantorOrganizationsServiceImpl;

@TestInstance(Lifecycle.PER_CLASS)
public class DMOGuaranteeOrgServiceTest {

	@InjectMocks
	private DMOGuarantorOrganizationsServiceImpl organizationsServiceImpl;

	@Mock
	private DMOGuarantorOrganizationsRepositry repository;

	@Mock
	private DMOGuarantorOrganizationsConverter convert;

	@Mock
	private DMOLoanGuarantorEntity entity;

	@Mock
	private DMOGuarantorOrganizationsDto basePostDto;
	...
	@Mock
	private Page<DMOLoanGuarantorEntity> dtlPage;
	@Mock
	private SpecificationImpl<DMOLoanGuarantorEntity> dtlSpec;
	...
	void testSaveAdvice() throws CustomException {
		basePostDto.setDepartmentId(1l);
		basePostDto.setDepartmentName("test");
		basePostDto.setOrganizationName("test");
		...
		assertNotNull(organizationsServiceImpl.saveOrUpdate(basePostDto));
	}
	...
	void getAllAdviceAgency() throws CustomException {
		...
		List<DMOLoanGuarantorEntity> dtlEntities = new ArrayList<>();
		dtlEntities.add(entity);
		List<DMOGuarantorOrganizationsDto> dtos = new ArrayList<>();
		dtos.add(basePostDto);
		when(convert.toDTO(dtlEntities)).thenReturn(dtos);
		assertNotNull(organizationsServiceImpl.gurantorOrgListing(pageDetail));
	}
}
