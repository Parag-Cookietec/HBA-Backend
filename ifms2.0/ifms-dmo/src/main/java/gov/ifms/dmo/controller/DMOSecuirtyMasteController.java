...
@RestController
@RequestMapping(DMOURLConstant.DMO_SECUIRTY_MASTER_BASE_URL)
@CrossOrigin(origins = "*", allowedHeaders = "*")
public class DMOSecuirtyMasteController {
	
	@Autowired
	DMOSecuirtyMasterService service;
	@Trace
	...
	public ResponseEntity<ApiResponse> adviceAgencyListing(@RequestBody PageDetails pageDetail) throws CustomException {
		return ResponseUtil.getResponse(HttpStatus.OK, MsgConstant.DP_SHEET_MSG_FETCH_LIST, service.secuirtyMasterListing(pageDetail));
	}

	@Trace
	@PostMapping(DMOURLConstant.URL_POST)
	public ResponseEntity<ApiResponse> saveorUpdate(@Valid @RequestBody DMOSecuirtyMasterDto dto) {
