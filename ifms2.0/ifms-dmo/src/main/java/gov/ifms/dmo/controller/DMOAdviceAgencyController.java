...
@RestController
@RequestMapping(DMOURLConstant.DMO_ADVICE_AGENCY_BASE_URL)
@CrossOrigin(origins = "*", allowedHeaders = "*")
public class DMOAdviceAgencyController {
	
	@Autowired
	DMOAdviceAgencyService service;
	...
	public ResponseEntity<ApiResponse> adviceAgencyListing(@RequestBody PageDetails pageDetail) throws CustomException {
		return ResponseUtil.getResponse(HttpStatus.OK, MsgConstant.DP_SHEET_MSG_FETCH_LIST, service.adviceAgencyListing(pageDetail));
	}

	
	@Trace
	@PostMapping(DMOURLConstant.URL_POST)
	public ResponseEntity<ApiResponse> saveorUpdate(@Valid @RequestBody DMOAdviceAgencyDto dto) {
