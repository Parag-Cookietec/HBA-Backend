...
import gov.ifms.gst.workflow.dto.AssignedToTrnDto;
...
import gov.ifms.gst.workflow.dto.WfRequestDto;
import gov.ifms.gst.workflow.dto.WfRlandPrDto;
...
/**
 * The Class GstWfActionConfigServiceImpl.
 */
@Service
public class GstWfActionConfigServiceImpl implements GstWfActionConfigService {

	Logger logger = LoggerFactory.getLogger(GstWfActionConfigServiceImpl.class);
	...
	@Override
	public List<GstWfActionConfigEntity> getNextWfActions(WfRequestDto dto) throws CustomException {
		EDPMsOfficeEntity office = officeRepo.getOne(dto.getOfficeId());

		Long entryCount;
		int isCoOffice = GstWorkFlowConstant.IS_CO_OFFICE_ZERO;
		int isToOffice = GstWorkFlowConstant.IS_TO_OFFICE_ZERO;
		if (office.getStatusId() == GstWorkFlowConstant.LU_APPROVED_LOOKUP_INFO_ID
				&& office.getOfficeStatus() == GstWorkFlowConstant.LU_ACTIVE_LOOKUP_INFO_ID
				&& office.getActiveStatus() == Constant.ACTIVE_STATUS) {
			if ((null != office.getIsCo()) && (office.getIsCo() == GstWorkFlowConstant.LU_YES_LOOKUP_INFO_ID)) {
				isCoOffice = GstWorkFlowConstant.IS_CO_OFFICE_ONE;
				...
				isToOffice = GstWorkFlowConstant.IS_TO_OFFICE_ONE;
			}
		}
		...
		if (entryCount != null && entryCount == 1) {
			if(office.getSubType()==null){
				return repository.findWorkflowRuleIdAndWfActionsWithSubTypeNull(office.getOfficeTypeId().getLookUpInfoId(),
						...
						assignedToDto.getAssignToActionLevel(), isCoOffice, GstWorkFlowConstant.DEFAULT_IS_CO, isToOffice,
						GstWorkFlowConstant.DEFAULT_IS_TO, Constant.ACTIVE_STATUS,
						GstWorkFlowConstant.DEFAULT_IS_DISPLAY_IN_POPUP);
			}else{
				return repository.findWorkflowRuleIdAndWfActionsWithSubType(office.getOfficeTypeId().getLookUpInfoId(),
						assignedToDto.getAssignToWfRoleId(), dto.getMenuId(), Constant.VALID_WF_ACTION_ID,
						assignedToDto.getAssignToActionLevel(), isCoOffice, GstWorkFlowConstant.DEFAULT_IS_CO, isToOffice,
						GstWorkFlowConstant.DEFAULT_IS_TO, Constant.ACTIVE_STATUS,
						GstWorkFlowConstant.DEFAULT_IS_DISPLAY_IN_POPUP,office.getSubType().getSubTypeId());
			}
		...
		if (trnDto.isPresent()) {
			...
			if(subTypeId==null) {
				initWfRlAndPr = repository.findWfInitWfRolesAndPrWithSubTypeNull(menuId, officeTypeId,
						Constant.ACTIVE_STATUS, isToOffice, GstWorkFlowConstant.DEFAULT_IS_TO);
			}else{
			...
			return new AssignedToTrnDto(finalInitWfRl, GstWorkFlowConstant.DEFAULT_WF_ACTION_LEVEL,
	...
	public List<WfUserDto> getUsersForInitiateWf(WfActConfUserRequestDto dto, GstWfActionConfigEntity wfActConf)
			throws CustomException {
		MenuTblInfoDto menuTblInfoDto = getTblInfo(wfActConf.getEdpMsMenuEntity().getMenuId());
		return repository
				.executeSQLQuery(
						GstQueryConstant.getUserForInitiatorWfQrString(menuTblInfoDto.getSchemaName(),
								menuTblInfoDto.getWfTblName(), wfActConf),
						GstQueryConstant.getUserForInitiatorWfQrParamMap(dto, wfActConf), WfUserDto.class);
	}
	...
	public List<WfActionConfigSubmitTrnResDto> submitWfActConfigTrn(WfActionConfigSubmitTrnDto dto)
		...
		repository.executeNativeUpdateSQLQuery(
				GstQueryConstant.updateActStsForWfTrnQrString(menuTblInfoDto.getSchemaName(), menuTblInfoDto.getWfTblName()),
				GstQueryConstant.updateActStsForWfTrnQrParamMap(dto.getWfDetails().get(0).getTrnId()));
		List<WfActionConfigSubmitTrnResDto> resDtoList = new ArrayList<>();
		dto.getWfDetails().forEach(wf -> {
			GstWfActionConfigEntity wfActConf = repository.findByActionConfigIdAndActiveStatus(wf.getActionConfigId(),
					Constant.ACTIVE_STATUS);
			Long wfId = repository.callWfSPLong(GstQueryConstant.getWfTrnSubmitSpQueryString(),
					GstQueryConstant.getWfTrnSubmitSpQueryMap(wf, wfActConf, decryptUserId(wf.getAssignedTo().getUserId()),
							decryptUserId(wf.getAssignedBy().getUserId())));
			try {
				resDtoList.add(repository.executeSQLQuerySingle(
						GstQueryConstant.getInsTrnDetailsQrStringQrString(menuTblInfoDto.getSchemaName(),
								menuTblInfoDto.getWfTblName()),
						GstQueryConstant.getInsTrnDetailsQrStringQrParamMap(wfId), WfActionConfigSubmitTrnResDto.class));
		...
		if (resDtoList.size() == 1) {
			if (!StringUtils.isEmpty(menuTblInfoDto.getWfAttTblName())) {
				repository.executeNativeUpdateSQLQuery(
						GstQueryConstant.updateTrnAttachQrStr(menuTblInfoDto.getSchemaName(), menuTblInfoDto.getWfAttTblName()),
						GstQueryConstant.updateTrnAttachQrParamMap(resDtoList.get(0).getWfId(),
			...
			if (!StringUtils.isEmpty(menuTblInfoDto.getWfMoveTblName())
				...
				repository.callWfStoredProcedure(
						GstDBConstants.GST_SCHEMA.concat(Constant.DOT.concat(GstWorkFlowConstant.SP_GST_WF_TRN_MV_LY_SUBMIT)),
						GstQueryConstant.getWfTrnMvLySpQueryMap(submitDto, resDtoList.get(0).getWfId(),
	...
	public int saveWfActConfigTrn(WfUserReqSDDto dto) throws CustomException {
		...
		if (assignedToDtoOp.isPresent()) {
			...
			if (pouId == dto.getPouId() || pouId == 0) {
				repository.executeNativeUpdateSQLQuery(
						GstQueryConstant.updateActStsForWfTrnQrString(menuTblInfoDto.getSchemaName(), menuTblInfoDto.getWfTblName()),
						GstQueryConstant.updateActStsForWfTrnQrParamMap(dto.getTrnId()));
				dto.setUserId(decryptUserId(dto.getUserId()));
				return repository.executeNativeUpdateSQLQuery(
						GstQueryConstant.updateAssignToTrnQrString(menuTblInfoDto.getSchemaName(), menuTblInfoDto.getWfTblName()),
						GstQueryConstant.updateAssignToTrnQrParamMap(dto, assignedToDtoOp.get()));
			} else {
			...
			dto.setUserId(decryptUserId(dto.getUserId()));
			return repository.executeNativeUpdateSQLQuery(
					GstQueryConstant.insTrnSaveQrStr(menuTblInfoDto.getSchemaName(), menuTblInfoDto.getWfTblName()),
					GstQueryConstant.insTrnSaveQrParamMap(dto, atDto));
		}
	}

	public List<WfUserDto> getUsersForForwardWf(WfActConfUserRequestDto dto, GstWfActionConfigEntity wfActConf) {
	...
	public Optional<AssignedToTrnDto> findActualWfRlIdAndLvlV2(String schemaName, String tblName, Long trnId,
		...
		List<AssignedToTrnDto> assignedToList = repository.executeSQLQuery(
				GstQueryConstant.getActualWfRoleIdWithAssignBy(schemaName, tblName),
				GstQueryConstant.getActTrnQrParamMap(trnId), AssignedToTrnDto.class);
	...
	public List<WfUserDto> getUsersForPreviousWf(WfActConfUserRequestDto dto, GstWfActionConfigEntity wfActConf)
			throws CustomException {
		MenuTblInfoDto menuTblInfoDto = getTblInfo(wfActConf.getEdpMsMenuEntity().getMenuId());
		return repository.executeSQLQuery(
				GstQueryConstant.getUserForPrevWfQrString(menuTblInfoDto.getSchemaName(), menuTblInfoDto.getWfTblName()),
				GstQueryConstant.getUserForPrevWfQrParamMap(dto, wfActConf), WfUserDto.class);
	}

	public List<WfUserDto> getUsersForReworkWf(WfActConfUserRequestDto dto, GstWfActionConfigEntity wfActConf)
			throws CustomException {
		MenuTblInfoDto menuTblInfoDto = getTblInfo(wfActConf.getEdpMsMenuEntity().getMenuId());
		return repository
				.executeSQLQuery(
						GstQueryConstant.getUserForReworkWfQrString(menuTblInfoDto.getSchemaName(), menuTblInfoDto.getWfTblName(),
								wfActConf),
					GstQueryConstant.getUserForReworkWfQrParamMap(dto, wfActConf), WfUserDto.class);
	}
